...
public class LogsController(ILogStore logStore, ILogger<LogsController> logger) : Controller
{
    ...
    public async Task<IActionResult> Index(string? q, string? level, DateTime? fromDate, DateTime? toDate, int page = 1, int pageSize = 50, CancellationToken cancellationToken = default)
    {
        ...
        catch (Exception ex)
        {
            ...
            TempData["ErrorMessage"] = "Không thể tải dữ liệu logs. Vui lòng thử lại sau.";
